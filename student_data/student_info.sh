...
# echo "$($PSQL "SELECT course FROM majors FULL JOIN students USING(major_id) FULL JOIN majors_courses USING(major_id) FULL JOIN courses USING(course_id) WHERE first_name = 'Obie' ORDER BY DESC")"
...
# echo "$($PSQL "SELECT course, COUNT(course_id) FROM students INNER JOIN majors USING(major_id) INNER JOIN majors_courses USING(course_id) INNER JOIN courses USING(course_id) GROUP BY course HAVING COUNT(student_id) = 1 ORDER BY course")"
